...
import WeiboPage from './components/WeiboPage';
...
function App() {
  // 切换展示的页面
  const [showOverview, setShowOverview] = useState(true); // 初始状态为显示 "Overview"
  const [showWeibo, setShowWeibo] = useState(false);
  const [showTwitter, setShowTwitter] = useState(false);
  // 饼图 + 播报栏
  const [tweetCount, setTweetCount] = useState(0);
  const [weiboCount, setWeiboCount] = useState(0);
  // 仪表盘
  const [tweetPositiveVal, setTweetPositiveVal] = useState(0);
  const [weiboPositiveVal, setWeiboPositiveVal] = useState(0);
  // 折线图相关
  const [xAxis, setXAxis] = useState([]);
  const [tweetDateVal, setTweetDateVal] = useState([]);
  const [weiboDateVal, setWeiboDateVal] = useState([]);
  const [tweetHotVal, setTweetHotVal] = useState([]);
  const [weiboHotVal, setWeiboHotVal] = useState([]);
  // 词云
  ...
  //   setShowWeibo(content === 'Weibo');
  //   setShowTwitter(content === 'Twitter');
  // };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://8.130.120.233:8088/overview');
        const allData = response.data.data;
        console.log(allData)
        // 大盘数据获取 + 初始化

        // 1. 饼图数据 + 播报栏数据
        const TweetCount = allData.barchart.TweetCount || 0;
        const WeiboCount = allData.barchart.WeiboCount || 0;

        // 2. 仪表盘数据
        const TweetPositiveVal = allData.linechart.TweetPositiveVal || 0;
        const WeiboPositiveVal = allData.linechart.WeiboPositiveVal || 0;

        // 3. 折线图数据 - 日期
        const TweetDateObj = allData.DateVal.TweetDateVal;
        const WeiboDateObj = allData.DateVal.WeiboDateVal;
        // 提取数值
        const TweetDateVal = Object.values(TweetDateObj);
        const WeiboDateVal = Object.values(WeiboDateObj);

        // console.log(TweetDateVal);
        var x_axis = Object.keys(TweetDateObj);
        // 处理数据
        x_axis = x_axis.map((val, index) => {
          const arr = val.split("-");
          var newVal = arr[1] + "-" + arr[2];
        ...
        // 4. 折线图数据 - 热度
        const TweetHotObj = allData.HotVal.TweetHotVal;
        const WeiboHotObj = allData.HotVal.WeiboHotVal;
        // 提取数值
        const tweetHotVal = Object.values(TweetHotObj);
        const weiboHotVal = Object.values(WeiboHotObj);

        // 5. 词云数据
        const overviewCloud = allData.OverviewCloud;

        // 设置数据：
        // 1. 设置 饼图数据 + 播报栏数据
        setTweetCount(TweetCount);
        setWeiboCount(WeiboCount);
        // todo: 播报栏的日期，5分钟自动更新 + 播报栏的样式要改一下

        // 2. 设置 仪表盘数据
        setTweetPositiveVal(TweetPositiveVal);
        setWeiboPositiveVal(WeiboPositiveVal);

        // 3. 设置折线图数据
        setXAxis(x_axis); // x坐标是通用的
        setTweetDateVal(TweetDateVal);
        setWeiboDateVal(WeiboDateVal)

        // 4. 折线图数据 - 热度
        setTweetHotVal(tweetHotVal);
        setWeiboHotVal(weiboHotVal);

        // 5. 词云数据
        setCloudData(overviewCloud);
  ...
  const hot_line = {
    title: {
      text: 'The Hot-Val of HKU-related Posts in Last Week',
      x: 'center',
      top: 10
    },
    grid: { top: 50, right: 30, bottom: 30, left: 50 },
  ...
  const option_grade_twitter = {
    title: {
      text: 'Sentiment_twitter',
      x: 'center',
      top: 10
    },
    series: [
  ...
  return (
    <div className='all-container'>
      ...
      <div className='notice-row'>
        <NoticeBoard totalNum={tweetCount + weiboCount} />
